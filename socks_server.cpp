...
class session : public std::enable_shared_from_this<session> {
  public:
    session(tcp::socket socket, boost::asio::io_context &io_context)
        : socket1_(std::move(socket)), socket2_(io_context),
  ...
  private:
    ...
    void firewall() {
        ...
        while (getline(in, rule)) {
            ...
            if (tmp[0] != "permit")
                continue;
            if (tmp[1] == "c" && request.cd == 2)
                continue;
            if (tmp[1] == "b" && request.cd == 1)
                continue;
            vector<string> ipX;
            vector<string> ipY;
    ...
    void parse(int length) {
        ...
        if (data_[4] == 0 && data_[5] == 0 && data_[6] == 0 && data_[7] != 0) {
            ...
            char ip[20];
            snprintf(ip, 20, "%d.%d.%d.%d", data_[4], data_[5], data_[6],
                     data_[7]);
